...
module.exports = (app, options) => {
  ...
  router.get('/cas/serviceValidate', async(ctx, next) => {
  ...
  router.get('/cas/proxy', async(ctx, next) => {
    if(!ctx.query){
  ...
  router.get('/cas/login', async(ctx, next) => {
    console.log('GET /cas/login');
    if(ctx.query && ctx.query.service){
      const pgtId = uuid.v4();
      tgts[pgtId] = initTgt();
      const st = 'ST-' + uuid.v4().substring(3);
  ...
  router.get('/cas/logout', async(ctx, next) => {
    ctx.body = 'ok';
  });

  router.post('/cas/v1/tickets', async(ctx, next) => {
  ...
  router["delete"]('/cas/v1/tickets/:tgt', async(ctx, next) => {
  ...
  router.get('/cas/v1/tickets', async(ctx, next) => {
